...
export async function renderIsoDemo(parent: HTMLElement, numProcs = 4): Promise<void> {
  ...
  for (const st of stationPositions(numProcs)) {
    ...
    add({ state: 'running', run: true }, st.x, st.y)
    for (let q = 0; q < 2; q++) add({ state: 'runnable', blink: q % 2 === 0 }, st.x - 6 - q * 5, st.y + 34 + q * 17)
  }
  ...
  ;[
    ...
    [230, 214],
    [214, 228],
  ].forEach((p) => add({ state: 'waiting', zzz: true, blink: true }, p[0], p[1]))
  ;[
    [372, 206],
    [406, 214],
    [390, 228],
  ].forEach((p, i) => add({ state: 'syscall', flip: i % 2 === 0, dots: true }, p[0], p[1]))
}
